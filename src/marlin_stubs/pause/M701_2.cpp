...
static bool load_unload(Pause::LoadType load_type, pause::Settings &rSettings) {
    ...
    #if HAS_AUTO_RETRACT()
        allow_cold = allow_cold || (is_unload && buddy::auto_retract().can_cold_unload(rSettings.physical_tool()));
    #endif
...
void filament_gcodes::M70X_process_user_response(PreheatStatus::Result res, VirtualToolIndex target_extruder) {
    ...
    case PreheatStatus::Result::DoneHasFilament: {
        ...
        if (config_store().cooldown_after_loading_when_idle.get() && marlin_server::printer_idle() && !marlin_server::printer_paused_extended()) {
            ...
            marlin_server::call_manually::set_temp_to_display(0, target_extruder.to_physical());
        } else {
            thermalManager.setTargetHotend(static_cast<int16_t>(disp_temp), target_extruder.to_physical());
            marlin_server::call_manually::set_temp_to_display(disp_temp, target_extruder.to_physical());
        }
        break;
    }
