...
internal G-code **M1987**, with results passed to the GUI via the extended-FSM-data channel
...
- `src/marlin_stubs/M1987.{hpp,cpp}` — the procedure/FSM driver.
...
- `marlin_stubs/gcode.cpp` — `case 1987`.
...
- `persistent_stores/.../config_store/store_definition.hpp` — `selftest_result_z_endstop_calibration`
  (`TestResult`, `ItemFlag::calibrations`); set by M1987 (passed if spread ≤ tolerance).
